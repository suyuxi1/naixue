	...
	<view class="container position-relative">
		<view style="margin-bottom: 130rpx">
					...
					<list-cell @click="chooseAddress">
						<view class="w-100 d-flex flex-column">
							<view class="d-flex align-items-center justify-content-between mb-10">
								<view class="font-size-extra-lg text-color-base">{{ choseAddress.street }}</view>
								<image src="/static/images/navigator-1.png" class="arrow"></image>
							</view>
							<view class="d-flex text-color-assist font-size-sm align-items-center">
								<view class="mr-10">{{ choseAddress.accept_name }}</view>
								<view class="mr-10">{{ !choseAddress.gender ? '先生' : '女士' }}</view>
			...
			<view class="section-2">
				<view class="cart d-flex flex-column">
					<list-cell last v-for="(item, index) in cart" :key="index">
						<view class="w-100 d-flex flex-column">
							...
							<view class="text-truncate font-size-base text-color-assist" v-if="item.props_text">{{ item.props_text }}</view>
						</view>
					</list-cell>
				...
				<list-cell last>
					<view class="flex-fill d-flex justify-content-end align-items-center">
						...
						<view class="font-size-extra-lg font-weight-bold">￥{{ totalFee }}</view>
					</view>
				</list-cell>
			</view>
			<!-- 购物车列表 end -->
			...
			<view class="payment">
				...
				<radio-group>
					<label class="d-flex align-items-center justify-content-between w-100 bg-white">
						<image src="../../static/images/pay/payment.png" class="image"></image>
		...
		<modal :show="ensureAddressModalVisible" custom :mask-closable="false" :radius="0" width="90%">
			<view class="modal-content">
				...
				<view class="d-flex font-size-base text-color-base font-weight-bold align-items-center justify-content-between mb-20">
					<view>{{ choseAddress.accept_name }} {{ choseAddress.gender ? '女士' : '先生' }}</view>
					<view>{{ choseAddress.mobile }}</view>
				</view>
				<view class="d-flex font-size-sm text-color-assist align-items-center justify-content-between mb-40">
					...
					<button type="primary" size="mini" plain class="change-address-btn" @tap="chooseAddress">修改地址</button>
				</view>
